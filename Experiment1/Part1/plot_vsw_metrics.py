...
- The MATLAB file Waveform_Reader_E1_part1.m uses manually-chosen time windows
  ...
  default windows.
- Some Tektronix exports may include only CH2 (no CH1). This script auto-detects
  the available channel column (CH1 or CH2).
"""
...
WINDOWS: dict[str, EdgeWindows] = {
    ...
    # Passive Probe with Spring Clip High-Z (T3)
...
def _select_vsw_column(df: pd.DataFrame, csv_path: Path) -> str:
    """Auto-detect which channel column holds Vsw.

    Preference:
    1) CH1 if present
    2) CH2 if present

    If neither exists, raise with a helpful message.
    """
    if "CH1" in df.columns:
        return "CH1"
    if "CH2" in df.columns:
        return "CH2"
    raise ValueError(f"Expected CH1 or CH2 column in {csv_path}, got {df.columns}")
...
def main():
    ...
    for csv_path in sorted(logs_dir.glob("*.csv")):
        ...
        vsw_col = _select_vsw_column(df, csv_path)
        print(f"[info] Using {vsw_col} as Vsw for {name}")

        time_s = df["TIME"].to_numpy(dtype=float)
        vsw_v = df[vsw_col].to_numpy(dtype=float)
